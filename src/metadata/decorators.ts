...
import { ConcreteType, Type, isFunction, stringify } from '@cashfarm/lang';
...
export function makePropDecorator(annotationCls: any): any {
  function PropDecoratorFactory(...args: any[]): any {
    let decoratorInstance = Object.create(annotationCls.prototype);
...
export function makeMethodDecorator(annotationCls: any): any {
  function MethodDecoratorFactory(...args: any[]): any {
    // var decoratorInstance = Object.create(annotationCls.prototype);
    const annotationInstance = new (<any>annotationCls)(...args);
    // annotationCls.apply(decoratorInstance, args);

    if (this instanceof annotationCls) {
      return annotationInstance;
    } else {
      return function MethodDecorator(target: any, name: string, descriptor: PropertyDescriptor): void {
        const meta = Reflect.getOwnMetadata('methodMetadata', target.constructor) || {};
        meta[name] = meta[name] || [];
        meta[name].unshift(annotationInstance);
        Reflect.defineMetadata('methodMetadata', meta, target.constructor);
      };
    }
  }

  MethodDecoratorFactory.prototype = Object.create(annotationCls.prototype);
  (<any>MethodDecoratorFactory).annotationCls = annotationCls;
